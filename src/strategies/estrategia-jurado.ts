...
export class EstrategiaJurado implements AuthenticationStrategy {
  name: string = 'jurado';
  ...
  async authenticate(request: Request): Promise<UserProfile | undefined> {
    ...
    if (tk) {
      ...
      let rolJurado = Configuracion.rolJurado;
      let urlToken = `${Configuracion.urlValidarToken}?${Configuracion.tokenArg}=${tk}&${Configuracion.rolArg}=${rolJurado}`
